...
import { sampleWithRequiredData } from '../inventory-item.test-samples';
...
describe('InventoryItem Management Component', () => {
  ...
  it('should call load all on init', () => {
    ...
    expect(comp.inventoryItems()[0]).toEqual(expect.objectContaining({ id: 7462 }));
  });

  describe('trackId', () => {
    it('should forward to inventoryItemService', () => {
      const entity = { id: 7462 };
      jest.spyOn(service, 'getInventoryItemIdentifier');
      const id = comp.trackId(entity);
      expect(service.getInventoryItemIdentifier).toHaveBeenCalledWith(entity);
