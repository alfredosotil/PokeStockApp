...
import { CardService, EntityArrayResponseType } from '../service/card.service';
...
export class CardComponent implements OnInit {
  ...
  trackId = (item: ICard): number => this.cardService.getCardIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.cards.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: ICard[] | null): ICard[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.cardService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
