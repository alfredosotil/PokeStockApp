 ...
 */
public class TradeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all trades").get("/api/trades").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new trade")
                        .post("/api/trades")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created trade").get("${new_trade_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created trade").delete("${new_trade_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Trade entity").exec(scn);
