...
public class PokeUserGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all pokeUsers").get("/api/poke-users").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new pokeUser")
                        .post("/api/poke-users")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"displayName\": \"SAMPLE_TEXT\"" + ", \"country\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_pokeUser_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created pokeUser").get("${new_pokeUser_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created pokeUser").delete("${new_pokeUser_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PokeUser entity").exec(scn);
