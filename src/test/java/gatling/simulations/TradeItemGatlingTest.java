...
public class TradeItemGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all tradeItems").get("/api/trade-items").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new tradeItem")
                        .post("/api/trade-items")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"quantity\": 0" + ", \"side\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_tradeItem_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created tradeItem").get("${new_tradeItem_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created tradeItem").delete("${new_tradeItem_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TradeItem entity").exec(scn);
